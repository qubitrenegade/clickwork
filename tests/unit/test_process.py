...
class TestRun:
    ...
    def test_passes_extra_env_vars(self):
        ...
        result = run(
            ["python3", "-c", "import os; print(os.environ['TEST_SECRET'])"],
...
class TestCapture:
    ...
    def test_always_executes_in_dry_run(self):
        ...
        output = capture(["echo", "data"], dry_run=True)
        assert output == "data"
...
class TestRunWithConfirm:
    ...
    def test_executes_when_confirmed(self):
        ...
        with patch("qbrd_tools.process._confirm_fn", return_value=True):
            result = run_with_confirm(["echo", "hello"], "Delete everything?")
    ...
    def test_yes_flag_bypasses_prompt(self):
        ...
        result = run_with_confirm(["echo", "hello"], "Delete?", yes=True)
        assert result is not None
        assert result.returncode == 0
    ...
    def test_dry_run_skips_execution(self):
        ...
        result = run_with_confirm(["false"], "Delete?", yes=True, dry_run=True)
        assert result is None
